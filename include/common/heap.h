...
#define _GET_PARENT_INDEX(index) ((index - 1) / 2)
  ...
  struct MinixHeap
  {
    struct HeapNode
    {
      struct NodeInfo : public std::enable_shared_from_this<NodeInfo>
      {
        ...
        ~NodeInfo() = default;
        uint32_t  heap_position_ = INVALID_HEAP_POSITION;
      ...
      /// @brief Move constructor. Transfer node_info_ pointer property to new node.
      ///        But not swap node_info_ value.
      /// @param node Move from node.
      HeapNode(HeapNode &&node);
      HeapNode &operator=(const HeapNode &node);

      /// @brief Swap node_info_ pointer property and node_info_ value. Move data_pair_ to new node.
      /// @param node: Move from node.
  ...
  void MinixHeap<KEY_T, CONTENT_T>::swap(MinixHeap<KEY_T, CONTENT_T>::HeapNode &node_a, MinixHeap<KEY_T, CONTENT_T>::HeapNode &node_b)
  {
    ...
    std::swap(node_a.data_pair_, node_b.data_pair_);
  }

  /// @brief Push a data pair instance to heap. Thread safe: no.
  /// @tparam KEY_T the type is used to determinate the order of this instance in heap.
  /// @tparam CONTENT_T node content.
  /// @param data_pair the whole data structure want to be contained in heap.
  /// @return PROCESS_SUCCESS: succeed to push to heap, PROCESS_FAILED: failed to push to heap.
  template<typename KEY_T, typename CONTENT_T>
  bool MinixHeap<KEY_T, CONTENT_T>::push(std::pair<KEY_T, CONTENT_T> &&data_pair)
  {
    ...
    return (heapifyFromChild(child_index) == INVALID_HEAP_POSITION) ? PROCESS_FAIL : PROCESS_SUCCESS;
  }
  ...
  std::optional<std::pair<KEY_T, CONTENT_T>> MinixHeap<KEY_T, CONTENT_T>::pop()
  {
    ...
    {
      return std::make_optional(*(ret.data_pair_));
    }
    heapify(0);
    return std::make_optional(*(ret.data_pair_));
  }
  ...
  bool MinixHeap<KEY_T, CONTENT_T>::erase(typename MinixHeap<KEY_T, CONTENT_T>::HeapNode &node)
  {
    ...
    return erase(node.node_info_->heap_position_);
  }

  template<typename KEY_T, typename CONTENT_T>
  MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode() :
    node_info_(std::make_shared<NodeInfo>()),
    data_pair_(std::make_shared<std::pair<KEY_T, CONTENT_T>>())
  {
  }
  ...
  template<typename KEY_T, typename CONTENT_T>
  uint32_t MaxHeap<KEY_T, CONTENT_T>::heapify(uint32_t parent_index)
  {
    auto end_index = this->heap_size_ - 1;
    while (1)
    {
      ...
      if (this->heap_[right_child_index].data_pair_->first > this->heap_[left_child_index].data_pair_->first)
      {
        if (this->heap_[right_child_index].data_pair_->first > this->heap_[parent_index].data_pair_->first)
        {
      ...
      {
        if (this->heap_[left_child_index].data_pair_->first > this->heap_[parent_index].data_pair_->first)
        {
  ...
  template<typename KEY_T, typename CONTENT_T>
  uint32_t MaxHeap<KEY_T, CONTENT_T>::heapifyFromChild(uint32_t child_index)
  {
    while (1)
    {
      if (child_index == 0)
      ...
      if (this->heap_[parent_index].data_pair_->first < this->heap_[child_index].data_pair_->first)
      {
  ...
  template<typename KEY_T, typename CONTENT_T>
  uint32_t MinHeap<KEY_T, CONTENT_T>::heapify(uint32_t parent_index)
  {
    auto end_index = this->heap_size_ - 1;
    while (1)
    {
  ...
  template<typename KEY_T, typename CONTENT_T>
  uint32_t MinHeap<KEY_T, CONTENT_T>::heapifyFromChild(uint32_t child_index)
  {
    while (1)
    {
      if (child_index == 0)

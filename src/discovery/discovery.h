...
struct sharedMemDiscovery : discovery
{
    ...
    sharedMemDiscovery(std::string_view sharedMemName, std::string_view topic, uint64_t participantId, identityMessageFormat::FLAG_TYPE participantFlag,
                       int sharedMemSize);
    virtual ~sharedMemDiscovery();
    int initialize(std::string_view sharedMemName, std::string_view topic, uint64_t participantId, identityMessageFormat::FLAG_TYPE participantFlag,
                   int sharedMemSize);
...
struct netDiscovery : discovery
{
    ...
    /// @brief create a discovery participant to discovery other remote
    /// participant and pronounce itself identity
    /// @param multicastGroupIp take part in a multicast group
    /// @param receiverLocalIp bind a ip address to receive multicast message,
    /// but it can be neglected.
    /// @param unicastIp ip address to bind unicast interface. e.g.
    /// "192.168.0.1:2152"
    /// @param topic used to subscribe or publish
    /// @param participantName identity information
    /// @param participantFlag used to define self role is a publisher or
    /// subscriber.
    netDiscovery(std::string_view multicastGroupIp, std::string_view receiverLocalIp, std::string_view unicastIp, std::string_view topic,
    ...
    netDiscovery(std::string_view multicastGroupIp, unsigned short monitorPort, std::string_view unicastIp, std::string_view topic,
                 std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag);

    virtual ~netDiscovery() = default;

    /// @brief initialize a discovery participant to discovery other remote
    /// participant and pronounce itself identity
    /// @param multicastGroupIp take part in a multicast group
    /// @param receiverLocalIp  bind a ip address to receive multicast message,
    /// but it can be neglected.
    ...
    int initialize(std::string_view multicastGroupIp, std::string_view receiverLocalIp, std::string_view unicastIp, std::string_view topic,
                   std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag);

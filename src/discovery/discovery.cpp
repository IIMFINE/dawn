...
netDiscovery::netDiscovery(std::string_view multicastGroupIp, std::string_view receiverLocalIp, std::string_view unicastIp, std::string_view topic,
                           std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag)
    : multicast_(std::make_shared<multicast>(multicastGroupIp, receiverLocalIp)), participantName_(participantName), topic_(topic),
      participantId_(std::hash<std::string_view>{}(participantName)), participantFlag_(participantFlag), multicastGroupIp_(multicastGroupIp)
{
    ...
    intrinsic_identity_sptr_ = std::shared_ptr<identityMessageFormat>(new (self_identity_mem) identityMessageFormat(topic_, participantId_, participantFlag),
                                                                      [](identityMessageFormat *mem) { freeMem(mem); });
...
netDiscovery::netDiscovery(std::string_view multicastGroupIp, unsigned short monitorPort, std::string_view unicastIp, std::string_view topic,
                           std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag)
    : multicast_(std::make_shared<multicast>(multicastGroupIp, monitorPort)), participantName_(participantName), topic_(topic),
      participantId_(std::hash<std::string_view>{}(participantName)), participantFlag_(participantFlag), multicastGroupIp_(multicastGroupIp)
{
    auto self_identity_mem = allocMem(sizeof(identityMessageFormat) + topic_.length());
...
int netDiscovery::initialize(std::string_view multicastGroupIp, std::string_view receiverLocalIp, std::string_view unicastIp, std::string_view topic,
                             std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag)
{
...
{
    std::shared_ptr<identityMessageFormat> message_sptr_(reinterpret_cast<identityMessageFormat *>(allocMem(UDP_MTU)), freeMem<identityMessageFormat>);
...
int netDiscovery::pronouncePresence(std::string_view port)
{
    ...
    if (multicast_->send2Socket(reinterpret_cast<char *>(intrinsic_identity_sptr_.get()), sizeof(identityMessageFormat) + topic_.length(), multicastGroupIp_) ==
        PROCESS_FAIL)
    {
        return PROCESS_FAIL;
    }
...
std::string_view identityMessageFormat::extractTopic() { return topicName_; }

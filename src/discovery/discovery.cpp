...
namespace dawn
{
    ...
    netDiscovery::netDiscovery(std::string_view multicastGroupIp, std::string_view receiverLocalIp,
                               std::string_view unicastIp, std::string_view topic, std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag) : multicast_(std::make_shared<multicast>(multicastGroupIp, receiverLocalIp)), participantName_(participantName), topic_(topic), participantId_(std::hash<std::string_view>{}(participantName)), participantFlag_(participantFlag), multicastGroupIp_(multicastGroupIp)
    {
    ...
    netDiscovery::netDiscovery(std::string_view multicastGroupIp, unsigned short monitorPort,
                               std::string_view unicastIp, std::string_view topic, std::string_view participantName, identityMessageFormat::FLAG_TYPE participantFlag) : multicast_(std::make_shared<multicast>(multicastGroupIp, monitorPort)), participantName_(participantName), topic_(topic), participantId_(std::hash<std::string_view>{}(participantName)), participantFlag_(participantFlag), multicastGroupIp_(multicastGroupIp)
    {
        auto self_identity_mem = allocMem(sizeof(identityMessageFormat) + topic_.length());
    ...
    identityMessageFormat::identityMessageFormat(std::string_view topic, uint64_t participantId, FLAG_TYPE flag) : topicLen_(topic.length()), participantId_(participantId), flag_(static_cast<unsigned char>(flag))
    {

...
public:
    multicast() = delete;
    /// @brief create sockets to send multicast datagram and receive multicast
    /// datagram,
    ///        this class will always create a local fd to send datagram.
    /// @param multicastGroupIpAddr if want don't need receive datagram socket
    /// and set INVALID_IP
    /// @param monitorPort use to bind local interface to receive multicast
    /// datagram, and it is
    ///                     same as the port of multicast IP.
    /// @param sendIpAddr if want don't need datagram socket bind a ipv4 address
    /// and set INVALID_IP.
    ...
    /// @brief join multicast group with a specific multicast ip and set local
    /// interface port

...
memoryPool::memoryPool() { storeEmptyLFQueue_.init(nullptr); }

void memoryPool::init()
{
    memoryNode_t *mallocMemory = nullptr;
    LF_Node<memoryNode_t *> *mallocLFNode = nullptr;
    for (int i = 0; i < kMaxMultiple; ++i)
    {
        int memBlockSize = kBaseBlockSize * (i + 1);
        int totalMemBlockNum = kEachBlockTotalNum.array[i];
        mallocMemory = (memoryNode_t *)malloc(totalMemBlockNum * (sizeof(memoryNode_t) + memBlockSize));
        mallocLFNode = new LF_Node<memoryNode_t *>[totalMemBlockNum];
        ...
        if (mallocMemory != nullptr && mallocLFNode != nullptr)
        {
            ...
            {
                mallocLFNode[j].element_val_ = reinterpret_cast<memoryNode_t *>(tempMallocMemory + (memBlockSize + sizeof(memoryNode_t)) * j);
                mallocLFNode[j].element_val_->memoryBlockType_ = i;
...
memoryNode_t *memoryPool::allocMemBlock(const int requestMemSize)
{
    ...
    LF_Node<memoryNode_t *> *memNodeContainer = memoryStoreQueue_[memMask].popNodeWithHazard();
    ...
    memoryNode_t *memNode = memNodeContainer->element_val_;
    memNodeContainer->element_val_ = nullptr;
...
bool memoryPool::freeMemBlock(memoryNode_t *releaseMemBlock)
{
    ...
    LF_Node<memoryNode_t *> *availableContain = nullptr;
    availableContain = storeEmptyLFQueue_.popNodeWithHazard();

    assert(availableContain != nullptr &&
           "error: no enough contain to save memory block and be care about some "
           "error happen");

    availableContain->element_val_ = releaseMemBlock;
    memoryStoreQueue_[memBlockType].pushNode(availableContain);
    return PROCESS_SUCCESS;
}

LF_Node<memoryNode_t *> *memoryPool::allocMemBlockWithContainer(const int requestMemSize)
{
    assert(requestMemSize > 0 && "error: requestMemSize is less than 0");
...
bool memoryPool::freeMemBlockWithContainer(LF_Node<memoryNode_t *> *releaseMemBlock)
{
    assert(releaseMemBlock != nullptr && "error: ask to release memory block is nullptr ");

    int memBlockType = releaseMemBlock->element_val_->memoryBlockType_;
    memoryStoreQueue_[memBlockType].pushNode(releaseMemBlock);
    return PROCESS_SUCCESS;
}

bool memoryPool::freeEmptyContainer(LF_Node<memoryNode_t *> *releaseContainer)
{
...
threadLocalMemoryPool::threadLocalMemoryPool() { initialize(); }
...
void *threadLocalMemoryPool::getMemBlockFromQueue(int blockType)
{
    ...
    auto memBlock = memContainer->element_val_;
    memContainer->element_val_ = nullptr;
...
bool threadLocalMemoryPool::recycleMemBlockToQueue(memoryNode_t *memBlock, int blockType)
{
    ...
    if (remainMemBlockContainerList_ != nullptr)
    {
        ...
        emptyContainer->element_val_ = memBlock;
    ...
    {
        auto emptyContainer = new LF_Node<memoryNode_t *>;
        emptyContainer->element_val_ = memBlock;
        emptyContainer->next_ = memBlockList_[blockType];
        memBlockList_[blockType] = emptyContainer;
        return PROCESS_SUCCESS;

...
#include "common/interprocess_utilities/notify/interprocessCondition.h"
...
TEST(test_interprocess_condition, wait_for_condition)
{
  ...
  std::thread t([&condition]() {
    ASSERT_EQ(condition.wait([]() {return true; }), PROCESS_SUCCESS);
    });
...
TEST(test_interprocess_condition, wait_for_condition_false)
{
  ...
  std::thread t([&condition]() {
    ASSERT_EQ(condition.wait([]() {return false; }), PROCESS_SUCCESS);
    });

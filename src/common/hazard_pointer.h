...
struct RegisterHazardToQueue
{
    ...
    RegisterHazardToQueue(std::shared_ptr<HazardPointerContainerHeader<T, ID_T>> &header, ID_T id) : local_container_(nullptr), queue_header_(header)
    {
    ...
    void insertHazardPointQueue()
    {
        ...
        while (1)
        {
            // When initializing hazard pointer queue, there are only push
            // operations.
    ...
    ~RegisterHazardToQueue() { local_container_->resetContainer(); }
};
...
struct HazardPointerContainer
{
    ...
    HazardPointerContainer(ID_T id) : next_(nullptr), id_(id) {}
    HazardPointerContainer(T value, ID_T id) : value_(value), next_(nullptr), id_(id) {}

    void setId(ID_T id) { id_.store(id, std::memory_order_release); }

    void setValue(T value) { value_.store(value, std::memory_order_release); }
    ...
    void resetValue() { value_.exchange(T()); }

    ~HazardPointerContainer() { this->resetContainer(); }
};
...
{
    for (auto container_index = this->getHeader()->next_.load(std::memory_order_acquire); container_index != nullptr; container_index = container_index->next_)
    {

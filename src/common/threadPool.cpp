...
namespace dawn
{
threadPool::~threadPool() { quitAllThreads(); }
...
void threadPool::runAllThreads() { run_thread_flag_ = ENUM_THREAD_STATUS::RUN; }

void threadPool::haltAllThreads() { run_thread_flag_ = ENUM_THREAD_STATUS::STOP; }
...
bool threadPool::popWorkQueue(FuncWrapper &taskNode)
{
    ...
    {
        taskNode = std::move(LF_stackNode->element_val_);
...
void threadPool::workThreadRun()
{
    for (; run_thread_flag_ != ENUM_THREAD_STATUS::EXIT;)
    {
        ...
        threadCond_.wait(stackLock,
                         [this]()
                         {
                             if ((taskNumber_.load(std::memory_order_acquire) > 0 && should_wakeup_.load(std::memory_order_acquire) == true) ||
                                 run_thread_flag_ == ENUM_THREAD_STATUS::EXIT)
                             {
                                 return true;
                             }

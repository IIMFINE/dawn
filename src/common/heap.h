...
#define _GET_PARENT_INDEX(index) ((index - 1) / 2)
    ...
    struct MinixHeap
    {
        struct HeapNode
        {
            ...
            /// @return PROCESS_SUCCESS: succeed to modify key, PROCESS_FAILED: failed to modify key.
            ...
            /// @return Position value. If node has been not in heap, it will return last position in heap.
            uint32_t getNodePosition()
            {
                return node_info_->heap_position_;
            }

            std::shared_ptr<std::pair<KEY_T, CONTENT_T>> getContent()
            {
                return data_pair_;
            }
        ...
        /// @return PROCESS_SUCCESS: succeed to heapify, PROCESS_FAILED: failed to heapify.
    ...
    template <typename KEY_T, typename CONTENT_T>
    MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode() : node_info_(std::make_shared<NodeInfo>()),
                                                        data_pair_(std::make_shared<std::pair<KEY_T, CONTENT_T>>())
    {
    }

    template <typename KEY_T, typename CONTENT_T>
    MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode(uint32_t heap_index, bool valid, std::pair<KEY_T, CONTENT_T> &&data_pair) : node_info_(std::make_shared<NodeInfo>(heap_index)),
                                                                                                                                data_pair_(std::make_shared<std::pair<KEY_T, CONTENT_T>>(std::forward<std::pair<KEY_T, CONTENT_T>>(data_pair)))
    {
    }

    template <typename KEY_T, typename CONTENT_T>
    MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode(uint32_t heap_index, std::pair<KEY_T, CONTENT_T> &&data_pair) : node_info_(std::make_shared<NodeInfo>(heap_index)),
                                                                                                                    data_pair_(std::make_shared<std::pair<KEY_T, CONTENT_T>>(std::forward<std::pair<KEY_T, CONTENT_T>>(data_pair)))
    {
    }

    template <typename KEY_T, typename CONTENT_T>
    MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode(const HeapNode &node) : node_info_(node.node_info_),
                                                                            data_pair_(node.data_pair_)
    {
    }

    template <typename KEY_T, typename CONTENT_T>
    MinixHeap<KEY_T, CONTENT_T>::HeapNode::HeapNode(HeapNode &&node) : node_info_(std::move(node.node_info_)),

...
struct IntervalUnit
{
    ...
    std::chrono::steady_clock::time_point getTimeOutValue() { return timeout_; }

    void setTimerOut(std::chrono::steady_clock::time_point timeout) { timeout_ = timeout; }
...
struct ConcreteIntervalUnit : public IntervalUnit
{
    ...
    ConcreteIntervalUnit(INTERVAL_T interval) : IntervalUnit(std::chrono::steady_clock::now() + interval), interval_(interval) {}

    std::chrono::nanoseconds getInterval() { return std::chrono::duration_cast<std::chrono::nanoseconds>(interval_); }
...
struct EventTimer : public std::enable_shared_from_this<EventTimer>
{
    struct EventHandle
    {
        ...
        bool init() { return PROCESS_SUCCESS; }

        bool destruct() { return PROCESS_SUCCESS; }
    ...
    EventTimer() : is_running_(false), min_interval_(std::chrono::seconds(1))
    {
            ...
            auto previous_items_num = items_num_.load(std::memory_order_acquire);
            {
                ...
                if (timer_callback_min_heap_->top().has_value())
                {
                    ...
                    if (now > timer_callback_min_heap_->top().value()->first->getTimeOutValue())
                    {
                        ...
                        heapNode.value().getContent()->first->setTimerOut(
                            now + std::chrono::duration_cast<std::chrono::nanoseconds>(heapNode.value().getContent()->first->getInterval()));

...
abstractSocket::abstractSocket(int bindFd, struct sockaddr_in clientIpAddr) : clientIpAddr_(clientIpAddr), bindFd_(bindFd) {}
...
std::string_view abstractSocket::getSourceIpAddr() { return sourceIpStr_; }

struct sockaddr_in abstractSocket::getClientIpAddr() { return clientIpAddr_; }

int abstractSocket::getBindFd() { return bindFd_; }
...
udpTransport::udpTransport(int bindFd, struct sockaddr_in clientIpAddr) : abstractSocket(bindFd, clientIpAddr) {}
...
tcpTransport::tcpTransport() { bindFd_ = INVALID_VALUE; }
...
tcpAcceptor::tcpAcceptor(int givenBindFd) : acceptFd_(INVALID_VALUE) { bindFd_ = givenBindFd; }
...
int tcpAcceptor::recvFromSocket(char *recvData, size_t dataBuffLen)
{
    ...
    // todo: change to use TLV method, must consider about image and other type
    // file transport, expect of text file.
...
epollMultiIOManager::epollMultiIOManager() : epollFd_(INVALID_VALUE), maxSpyFdNum_(1) {}

epollMultiIOManager::~epollMultiIOManager() { fdSocketMap_.clear(); }
...
int epollMultiIOManager::setEventProcessor(eventProcessSubject *) { return PROCESS_SUCCESS; }

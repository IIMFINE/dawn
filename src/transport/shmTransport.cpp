...
namespace dawn
{
    ...
    shmIndexRingBuffer::shmIndexRingBuffer(std::string_view identity) : shmIdentity_(identity),
    ...
    shmMsgPool::shmMsgPool(std::string_view identity) : defaultPriority_(0),
                                                        identity_(identity),
                                                        mechanismIdentity_(kMechanismPrefix + identity_),
                                                        mqIdentity_(kMessageQueuePrefix + identity_)
    {
        using namespace BI;
        ipc_ptr_ = std::make_shared<interprocessMechanism<IPC_t>>(mechanismIdentity_);
    ...
    {
        return tpController_ptr_->read(read_data, data_len, BLOCKING_TYPE::BLOCK);
    }

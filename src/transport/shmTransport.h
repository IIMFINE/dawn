...
namespace dawn
{
/// @brief shm ring buffer block content: |shm_message_index, message_size, time
/// stamp|...|...|...
...
///    because of thinking about the scenario for one publisher to multiple
///    subscribers.
struct shmIndexRingBuffer
{
    ...
    /// @brief Start index move a step and return content pointed by previous
    /// start index.
    /// @param prevIndex
    ...
    /// @brief Watch start index and lock start index mutex until operation
    /// finish.
    /// @param indexBlock
    /// @return PROCESS_SUCCESS: lock successfully and read ring buffer in start
    /// index.
    ...
    /// @brief Watch end index and lock end index mutex until watch operation
    /// asked to stop.
    /// @param indexBlock
    /// @return PROCESS_SUCCESS: lock successfully and read buffer.
    ///         PROCESS_FAIL: buffer is empty or buffer isn't initialize and
    ///         will not lock.
    ...
    /// @brief Watch specific index and lock specific index mutex until watch
    /// operation asked to stop.
    /// @param index Pass specific index to watch.
    /// @param indexBlock Outside buffer to store index block content.
    /// @return PROCESS_SUCCESS: lock successfully and read buffer.
    ///         PROCESS_FAIL: index is invalid or buffer isn't initialize and
    ///         will not lock.
    ...
    /// @brief Pass indexBlock to compare the the block pointing by start index.
    /// If they are same, start index will move.
    /// @param indexBlock
...
struct shmTransport : abstractTransport
{
    ...
    /// @brief Read data from shared memory. It can choose blocking or
    /// non-blocking.
    /// @param read_data read data buffer
    /// @param data_len read data length
...
bool shmChannel::tryWaitNotify(FUNC_T &&func, uint32_t microsecond)
{
    ...
    return ipc_ptr_->mechanism_raw_ptr_->condition_.timed_wait(
        lock, boost::posix_time::microsec_clock::universal_time() + boost::posix_time::microseconds(microsecond), func);
}

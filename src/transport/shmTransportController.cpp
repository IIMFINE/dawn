...
namespace dawn
{
    efficientTpController_shm::efficientTpController_shm(std::unique_ptr<shmTransportImpl> &&shmImpl) : shmImpl_(std::forward<std::unique_ptr<shmTransportImpl>>(shmImpl))
    {
    }

    efficientTpController_shm::efficientTpController_shm(std::unique_ptr<shmTransportImpl> &&shmImpl, std::any config) : qosCfg_(std::any_cast<qosCfg>(config)),
    ...
    reliableTpController_shm::reliableTpController_shm(std::unique_ptr<shmTransportImpl> &&shmImpl) : shmImpl_(std::forward<std::unique_ptr<shmTransportImpl>>(shmImpl))
    {
    }

    reliableTpController_shm::reliableTpController_shm(std::unique_ptr<shmTransportImpl> &&shmTp, std::any config) : qosCfg_(std::any_cast<qosCfg>(config)),
